...
class Country:
    ...
    def __getattr__(self, __name: str) -> str:
        if __name in Country.index:
            return self.__attr[Country.index[__name]]
        else:
            raise(AttributeError)
 
    # TODO: implement an attribute delete function
    def __delattr__(self, __attr: str) -> None:
        if __attr in Country.index:
            deleted_field = self.__attr[Country.index[__attr]]
            if isinstance(deleted_field, str):
                self.__attr[Country.index[__attr]] = ""
            else:
                self.__attr[Country.index[__attr]] = 0
        else:
            print("No such element with that name.")            
